describe('Basic user flow for Website', () => {
  ...
  it('Clicking the "Add to Cart" button should change button text', async () => {
    ...
    const textAfter = await button.getProperty('innerText')
    const text = await textAfter.jsonValue();
  ...
  it('Checking number of items in cart on screen', async () => {
    ...
    const cartProp = await cart.getProperty('innerText');
    const cartCount = await cartProp.jsonValue();

    expect(cartCount).toBe('20');
    
  }, 100000); //! FIX TIMEOUT LIMIT MAYBE
